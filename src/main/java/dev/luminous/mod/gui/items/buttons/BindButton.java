...
extends Button {
    ...
    public void drawScreen(class_332 context, int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.isListening) {
            this.drawString("Press a Key...", (double)(this.x + 2.3f), (double)(this.y - 1.7f - (float)ClickGuiScreen.getInstance().getTextOffset()), enableTextColor);
        } else {
            String str = this.setting.getKeyString();
            if (this.isListening || !this.isHovering(mouseX, mouseY) || !class_3675.method_15987((long)mc.method_22683().method_4490(), (int)340) || !this.setting.getName().equals("Key")) {
                this.drawString(this.setting.getName() + " " + String.valueOf(class_124.field_1080) + str, (double)(this.x + 2.3f), (double)(this.y - 1.7f - (float)ClickGuiScreen.getInstance().getTextOffset()), this.getState() ? enableTextColor : defaultTextColor);
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        if (mouseButton == 0 && this.isHovering(mouseX, mouseY)) {
            if (class_3675.method_15987((long)mc.method_22683().method_4490(), (int)340) && this.setting.getName().equals("Key")) {
