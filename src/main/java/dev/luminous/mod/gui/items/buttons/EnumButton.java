...
extends Button {
    ...
    public void drawScreen(class_332 context, int mouseX, int mouseY, float partialTicks) {
        ...
        this.drawString(this.setting.getName() + " " + String.valueOf(class_124.field_1080) + (((Enum)this.setting.getValue()).name().equalsIgnoreCase("ABC") ? "ABC" : ((Enum)this.setting.getValue()).name()), (double)(this.x + 2.3f), (double)(this.y - 1.7f - (float)ClickGuiScreen.getInstance().getTextOffset()), this.getState() ? enableTextColor : defaultTextColor);
        if (this.open) {
            int y = (int)this.y;
