...
extends Module {
    ...
    public void onDeath(DeathEvent event) {
        ...
        if (player != AutoEZ.mc.field_1724 && !Alien.FRIEND.isFriend(player)) {
            if (this.range.getValue() > 0.0 && (double)AutoEZ.mc.field_1724.method_5739((class_1297)player) > this.range.getValue()) {
                return;
            }
            ...
            switch (this.type.getValue().ordinal()) {
                case 0: {
                    mc.method_1562().method_45729(this.bot.get(this.random.nextInt(this.bot.size() - 1)) + " " + player.method_5477().getString() + (String)randomString);
                    break;
                }
                case 1: {
                    mc.method_1562().method_45729(this.message.getValue().replaceAll("%player%", player.method_5477().getString()) + (String)randomString);
                    break;
                }
                case 2: {
                    mc.method_1562().method_45729(this.sex.get(this.random.nextInt(this.sex.size() - 1)) + " " + player.method_5477().getString() + (String)randomString);
                }
            }
        }
    }
